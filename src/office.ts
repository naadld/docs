...
/**
 * Trigger GitHub Actions Runner Engine via repository_dispatch API
 */
export async function handleOfficeAction(
  ...
  await sendTelegramMessage(
    env,
    chatId,
    `🚀 <b>HaRI GitHub:</b> Triggering <b>[${actionTitle}]</b> for <code>${escapeHtml(fileName)}</code>...`,
    'HTML',
    undefined,
    threadId
  );
  ...
  try {
    ...
    if (res.ok || res.status === 204) {
      await sendTelegramMessage(
        env,
        chatId,
        `⚡ <b>Runner Dispatched!</b> Processing <code>${escapeHtml(fileName)}</code> with LibreOffice & pdftoppm. Output will arrive shortly!`,
        'HTML',
        undefined,
        threadId
      );
